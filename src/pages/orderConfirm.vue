  ...
  methods:{
    ...
    submitAddress(){
      ...
      if(userAction == 0 || userAction == 1){
        let {receiverName, receiverMobile, receiverProvince, receiverDistrict, receiverCity, receiverAddress, receiverZip} = checkedItem;
        let errMsg;
        //表单校验
        if(!receiverName){
          errMsg = '请输入收货人名称';
    ...
    getCartList(){
      this.axios.get('/carts').then((res)=>{
       ...
       this.cartTotalPrice = res.cartTotalPrice;//已经选中的商品总金额
       this.cartList = list.filter(item=>item.productSelected);//过滤出来已选中的商品
       list.map((item)=>{
